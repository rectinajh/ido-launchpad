...
import OKXcon from "../../assets/images/OKX.svg";
...
export default function WalletModal(props) {
  ...
  useEffect(
    () => {
      ...
      console.log("networks", networks, SUPPORTED_NETWORKS);
  ...
  const tryActivation = async (connector) => {
    ...
    // if (isMobile && !isOKApp){
    //   const encodedUrl = "https://www.okx.com/download?deeplink=" + encodeURIComponent("okx://wallet/dapp/url?dappUrl=" + encodeURIComponent(window.location.href));
  ...
  function getAvailableWallets() {
    const availableOptions = Object.keys(SUPPORTED_WALLETS).map((key) => {
      ...
      if (!window.okxwallet &&!option.mobileOnly) {
        return (
          <Option
            id={`connect-${key}`}
            key={key}
            ...
            subheader={null}
            link={"https://www.okx.com/en/web3"}
            icon={OKXcon}
            size={45}
          />
        );
